...
public class JdbcRepository {
    ...
    public void update(Person person) {
        Connection con = null; // 나중에 close 하려고 미리 선언함
    ...
    public List<Person> findAll() {
        ...
        try {
            ...
            // 만약 내가 작성한 sql의 조회 결과가 여러 행이라면 반복문을 이용해서 한 행식 데이터를 가져온다
            // 그리고 더 잇아 조회할 데이터가 없으면 종료되도록 설계한다
            while (rs.next()) {
                // 특성 한 행이 지목된 상태 -> 행의 컬럼을 지목해서 데이터를 가져온다 (타입 확인 필요)
                int id = rs.getInt("id");
